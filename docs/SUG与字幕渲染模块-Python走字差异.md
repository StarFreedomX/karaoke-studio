...
2026-08 起（SUG v1.6.2 `ba1d6a74`，字幕模块同步修复）：空格等零墨水字符
（无渲染字符）权重为 0——得到零时长窗口、瞬时跨过，时长由可见字符瓜分；
...
`len(reading_parts) == len(reading_part_ms) + 1`。2026-07-14 起，普通横排、RTL、
竖排和字符转场路径不再把一个标量 ratio 乘到整块 ruby 目标盒，而是：

- 保留每个原始 part 的 checkpoint 区间；
- 将非空 part 按可见 Unicode 字符拆分，浊点/半浊点附着前字，小假名保持独立；
- 多字符 part 使用整数边界均分自身时间段；
- 每个字符的 wipe 从实际 `QPainterPath` 墨水左缘走到右缘（RTL 反向，竖排为上到下）；
- 相邻字形之间的透明间隙在字符切换时跳过，目标盒两侧的居中留白只参与排版、不消耗演唱时间；
- 空 part 仍消耗时间，但不生成几何段，因此锋面保持不动。

这与 NicoKaraMaker3 的 `CalcActualTimes` + `SetOneLineWipe` 语义一致：时间属于 ruby
字符，wipe 属于该字符的实际绘制几何。`_ruby_progress_ratio()` 仅保留给旧数据/诊断回退，
不再驱动标准 ruby 图层的裁剪。
...
- 旧实现：目标槽前后留白被算进连续扫光距离，可能表现为 ruby 很晚才开始或提前走完；
- 当前实现：首字符从自身墨水边缘立即开始，末字符到自身墨水边缘才完成。

剩余差异主要来自两边字体栅格化、连词范围重建和 LRC 厘秒量化，而不是目标盒留白或
part 权重公式。
...
字符级 ruby 几何随 `_LineLayout` 一起缓存，不按帧重复测量。before/after 位图缓存键包含
相对字形位置签名，因此 Center/EqualSpace 即使目标宽度和读音总宽相同也不会误复用；
绝对行坐标和当前 `t_ms` 仍不进入位图键，平移与逐帧裁剪可以继续复用同一份静态图层。
